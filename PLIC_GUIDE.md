✦ A PLIC (Platform-Level Interrupt Controller) acts as a central hub that aggregates many interrupt sources (from peripherals) and routes them to specific "targets" (processor contexts).
...
✦ In a standard Linux-compatible RISC-V SoC (following the SiFive/RISC-V PLIC specification), the "Targets" (often called "Contexts") are mapped to the privilege modes of the CPU cores.
  ...
  ├────────────────────┼─────────────────────────┼────────────────────────────┤
  │ Interrupt Enabless │ 0x002000                │ 0x002080                   │
  ...
  To ensure Linux recognizes this mapping, your Device Tree (.dts) should specify the interrupts for the CPU core as follows:
   ...
   1 plic0: interrupt-controller@40000000 {
   2     compatible = "sifive,plic-1.0.0";
   3     #interrupt-cells = <1>;
   4     interrupt-controller;
   5     interrupts-extended = <&cpu0_intc 11 &cpu0_intc 9>;
   6     reg = <0x0 0x40000000 0x0 0x400000>;
   7     riscv,ndev = <31>;
   8 };
