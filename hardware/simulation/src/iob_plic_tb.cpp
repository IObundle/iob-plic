...
#define PRIORITY_REGS                                                          \
  ((SOURCES + PRIORITY_FIELDS_PER_REG - 1) / PRIORITY_FIELDS_PER_REG)
...
int test_simple_target_source() { // check if there are any interrupts pending
  ...
  for (target = 0; target < TARGETS; target++) {
    for (source = 0; source < SOURCES; source++) {
      ...
      // enable interrupt
      // EDGE_LEVEL_REGS is used, as it holds the amount of IE-registers per
      // target
      err = set_inputs(
      ...
      err = set_inputs((id_base_address + target) * DATA_W / 8,
                       1 << (source % DATA_W), 0xF);
...
int write_pr_regs() {
  ...
  // printf("%x -> %x", res, 0x11111111); // check if correct considering
  // PRIORITY_FIELDS_PER_REG=8 and PRIORITY_NIBBLES=1
  return res;
}